...
svg* svg_create(int width,int heigth){
    svg* psvg = malloc(sizeof(svg));

    if(psvg != NULL){
        *psvg = (svg){.svg = NULL,.width = width,.height = heigth,.finalized = false};
        psvg->svg = malloc(1);
...
void svg_rectangle(svg* psvg,int w,int h,int x,int y,char* fill,char* stroke,int strokewidth,int radiusx,int radiusy){
    ...
    appendnumbertosvg(psvg, w);
    appendstringtosvg(psvg, "' height='");
    appendnumbertosvg(psvg, h);
